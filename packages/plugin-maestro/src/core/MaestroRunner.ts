...
import type { MaestroRunOptions, MaestroRunResult } from './types.js';
 ...
 *
 * @example
 * const res = await new MaestroRunner().run('tests/maestro/flows/android/settings.yaml', {
...
export class MaestroRunner {
  ...
  async run(flowPath: string, options: MaestroRunOptions): Promise<MaestroRunResult> {
    ...
    const env = options.platform === 'android' ? getPlatform().androidEnv() : process.env;
    ...
    return new Promise<MaestroRunResult>((resolve, reject) => {
      ...
      // above are absolute, so cwd doesn't affect them; the binary is pre-resolved because a `cwd`
      // breaks Node's PATH lookup of a bare name.
